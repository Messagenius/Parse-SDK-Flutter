  ...
  Future<void> _init() async {
    ...
    LiveQuery()
      ...
      subscription.on(
        ...
        (T object) => _updateQueue.whenComplete(() => _objectUpdated(object)),
      );
      subscription.on(
      ...
      subscription.on(
        ...
        (T object) => _updateQueue.whenComplete(() => _objectDeleted(object)),
      );
      subscription.on(
            ...
            for (int i = 0; i < _list.length; i++) {
              ...
              for (int j = 0; j < newList.length; j++) {
                if (newList[j].get<String>(keyVarObjectId) == currentObjectId) {
                  stillInList = true;
                  if (newList[j].get<DateTime>(keyVarUpdatedAt)!.isAfter(
